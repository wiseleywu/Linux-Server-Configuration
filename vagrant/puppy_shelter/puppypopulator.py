...
#Add Shelters
shelter1 = Shelter(name = "Oakland Animal Services", address = "1101 29th Ave", city = "Oakland", state = "California", zipCode = "94601", website = "oaklandanimalservices.org",capacity = 30, occupancy = 0)
session.add(shelter1)

shelter2 = Shelter(name = "San Francisco SPCA Mission Adoption Center", address="250 Florida St", city="San Francisco", state="California", zipCode = "94103", website = "sfspca.org", capacity = 30, occupancy = 0)
session.add(shelter2)

shelter3 = Shelter(name = "Wonder Dog Rescue", address= "2926 16th Street", city = "San Francisco", state = "California" , zipCode = "94103", website = "http://wonderdogrescue.org", capacity = 30, occupancy = 0)
session.add(shelter3)

shelter4 = Shelter(name = "Humane Society of Alameda", address = "PO Box 1571" ,city = "Alameda" ,state = "California", zipCode = "94501", website = "hsalameda.org", capacity = 30, occupancy = 0)
session.add(shelter4)

shelter5 = Shelter(name = "Palo Alto Humane Society" ,address = "1149 Chestnut St." ,city = "Menlo Park", state = "California" ,zipCode = "94025", website = "paloaltohumane.org", capacity = 30, occupancy = 0)
...
puppy_description = ['adaptive','babyish','badass','classy','clingy','flawed',
                     'flexible','great','needy','outward','perfect','reactive',
                     'sad','sporty','territorial','tidy','active','adventurous',
                     'affectionate','aggressive','annoying','brave','cheerful',
                     'confident','cowardly','decisive','determined','easygoing',
                     'enthusiastic']
...
def checkIn(puppy, shelter_id):
    shelter=session.query(Shelter).filter(Shelter.id==shelter_id).first()
    if shelter.capacity==shelter.occupancy:
        print "Sorry, this shelter is full. Try another shelter"
    else:
        shelter.occupancy+=1
        session.add_all([puppy,shelter])
        session.commit()

def checkOut(puppy_id, adopters):
    puppy=session.query(Puppy).filter(Puppy.id==puppy_id).first()
    if  len(puppy.adopter)>0:
        print "Sorry, this puppy has been adopted already"
    else:
        for adopter_id in adopters:
            owner=session.query(Adopter).filter(adopter_id==Adopter.id).first()
            puppy.adopter.append(owner)
        shelter=session.query(Shelter).filter(Shelter.id==puppy.shelter_id).first()
        shelter.occupancy-=1
        session.add_all([puppy, shelter])
        session.commit()
...
def populate():
    for i,x in enumerate(male_names):
        ...
        new_puppy = Puppy(name = x, gender = "male", dateOfBirth = CreateRandomAge(),picture=random.choice(puppy_images) ,shelter_id=shelter_id, weight= CreateRandomWeight())
        new_puppy.profile=[Profile(description=random.choice(puppy_description))]
        checkIn(new_puppy,shelter_id)
        # new_profile = Profile(description=random.choice(puppy_description))
